...
class CartaVuelo:
    ...
    def draw(self, screen):
        ...
        text = self.font.render(f"{self.nodo_vuelo.get_id()}/{self.vuelos.n_nodos}", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y - 250))
        screen.blit(text, text_rect)
        text = self.font.render(f"Origen: {self.nodo_vuelo.vuelo.origen}", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y - 150))
        screen.blit(text, text_rect)
        text = self.font.render(f"Destino: {self.nodo_vuelo.vuelo.destino}", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y - 90))
        screen.blit(text, text_rect)
        text = self.font.render(f"Trimestre: {self.nodo_vuelo.vuelo.trimestre}", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y - 30))
        screen.blit(text, text_rect)
        text = self.font.render(f"Compañía: {self.nodo_vuelo.vuelo.compania}", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y + 30))
        screen.blit(text, text_rect)
        text = self.font.render(f"Distancia: {self.nodo_vuelo.vuelo.kilometros}km", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y + 90))
        screen.blit(text, text_rect)
        text = self.font.render(f"Precio: {self.nodo_vuelo.vuelo.precio}€", True, self.colors["WHITE"])
        text_rect = text.get_rect(center = (x, y + 150))
